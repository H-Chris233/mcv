...
package app.multicardvault.uniffi
...
private fun uniffiCheckContractApiVersion(lib: IntegrityCheckingUniffiLib) {
    ...
    val scaffolding_contract_version = lib.ffi_mcv_uniffi_uniffi_contract_version()
...
private fun uniffiCheckApiChecksums(lib: IntegrityCheckingUniffiLib) {
    ...
    if (lib.uniffi_mcv_uniffi_checksum_func_decode_vault_plaintext() != 34165.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    ...
    if (lib.uniffi_mcv_uniffi_checksum_func_mcv_project_status() != 48212.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_mcv_uniffi_checksum_func_unlock_vault() != 17411.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_mcv_uniffi_checksum_func_update_vault() != 36238.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
}
...
data class CreateVaultRequest (
     ...
     */
    var `deviceSecret`: kotlin.ByteArray,
    /**
...
data class CreateVaultResponse (
     ...
     * Encoded `VaultBlobV1`.
     */
    var `vaultBlob`: kotlin.ByteArray,
    /**
...
public object FfiConverterTypeMcvFfiError : FfiConverterRustBuffer<McvFfiException> {
    override fun read(buf: ByteBuffer): McvFfiException {
        ...
        return when(buf.getInt()) {
            ...
            4 -> McvFfiException.InvalidSchemeId()
            5 -> McvFfiException.DuplicateShareIndex()
            6 -> McvFfiException.NotEnoughShares()
            7 -> McvFfiException.InvalidThreshold()
            8 -> McvFfiException.InvalidDeviceSecret()
            9 -> McvFfiException.InvalidCardPayload()
            10 -> McvFfiException.InvalidVaultBlob()
            11 -> McvFfiException.InvalidVaultPlaintext()
            12 -> McvFfiException.CardAuthenticationFailed()
            13 -> McvFfiException.VaultAuthenticationFailed()
            14 -> McvFfiException.CryptoException()
    ...
    override fun allocationSize(value: McvFfiException): ULong {
        return when(value) {
            ...
            is McvFfiException.InvalidMagic -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidVaultId -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidSchemeId -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.DuplicateShareIndex -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.NotEnoughShares -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidThreshold -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidDeviceSecret -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidCardPayload -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidVaultBlob -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.InvalidVaultPlaintext -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.CardAuthenticationFailed -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.VaultAuthenticationFailed -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.CryptoException -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
            is McvFfiException.ShamirException -> (
                // Add the size for the Int that specifies the variant plus the size needed for all fields
                4UL
            )
        }
...
public object FfiConverterSequenceTypeVaultPlaintextEntry: FfiConverterRustBuffer<List<VaultPlaintextEntry>> {
    override fun read(buf: ByteBuffer): List<VaultPlaintextEntry> {
        ...
        return List<VaultPlaintextEntry>(len) {
            FfiConverterTypeVaultPlaintextEntry.read(buf)
        }
    }

    override fun allocationSize(value: List<VaultPlaintextEntry>): ULong {
        val sizeForLength = 4UL
        val sizeForItems = value.map { FfiConverterTypeVaultPlaintextEntry.allocationSize(it) }.sum()
        return sizeForLength + sizeForItems
    }
